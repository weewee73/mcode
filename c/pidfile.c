...
int write_pid(char *pidfile)
{
    ...
    pid = getpid();
    if (!fprintf(f,"%d\n", pid)) {
        printf("Can't write pid , errno=%d\n", errno);
        fclose(f);
        return 0;
    }
    fflush(f);
    ...
    if (flock(fd, LOCK_UN) == -1) {
        printf("Can't unlock pidfile %s, %d\n", pidfile, errno);
        fclose(f);
        return 0;
    }
#endif
...
#include <stdlib.h>
#define PidFile "/var/run/testd.pid"

int main()
{

    printf("Writing pidfile '%s'.\n", PidFile);
    ...
    {
        fprintf(stderr, "testd: pidfile '%s' and pid %d already exist.\n",
                PidFile, getpid());
        exit(1); /* exit during startup - questionable */
    }
