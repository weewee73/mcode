...
cvs_add_all() {
    [ -z "$1" ] && { echo "error: dir is null" ; return 1; }

    cvs add $(find $1 -type d \! -name CVS) 2>/dev/null
    cvs add $(find $1 \( -type d -name CVS -prune \) -o \( -type f \))
}
...
corefile_name() {
    for file ; do
        echo -n $file : ; gdb --core=$file --batch | grep 'Core was generated by'
    done
}
